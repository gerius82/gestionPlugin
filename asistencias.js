...
function mostrarAlumnos(turno, sede) {
  ...
  alumnosDelTurno.forEach(alumno => {
    ...
    //li.innerHTML = `<label><input type="checkbox" name="presente" value="${alumno.id}" checked> ${alumno.nombre} ${alumno.apellido}</label>`;
    li.innerHTML = `
    <label>
...
function guardarAsistencia(fecha, turno, sede) {
  const presentes = [...document.querySelectorAll("input[name='presente']:checked")].map(el => el.value);
  const registros = [
    ...presentes.map(id => ({ alumno_id: id, fecha, turno, sede, tipo: "regular" })),
