...
.then(cfg => {
  ...
  async function cargarTurnosPorSede(sede) {
    ...
    const turnos = Object.entries(cuposMaximos[sede]);

    // Preparar todas las promesas de consulta
    const consultas = turnos.map(([turno, max]) => {
      const turnoLimpio = turno.replace(/hs$/, "").trim();
      return contarInscriptos(sede, turno).then(cantidad => ({
        turno,
        max,
        cantidad
      }));
    ...
    resultados.forEach(({ turno, max, cantidad }) => {
      const listaEspera = Number(cantidad) >= Number(max);
      const label = listaEspera ? `${turno} - Lista de espera` : turno;

      turno1.appendChild(new Option(label, turno));
      turno2.appendChild(new Option(label, turno));
    });
